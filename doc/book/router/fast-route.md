...
into the `Mezzio\Router\FastRoute` class, at instantiation.
...
`Mezzio\Router\FastRoute` during instantiation.
...
inject them in your `Mezzio\Router\FastRoute` instance, and inject use
...
use Mezzio\Router\FastRoute as FastRouteBridge;
...
instantiate and return an instance of `Mezzio\Router\FastRoute` for the
...
- A factory registered as `Mezzio\Router\RouterInterface`, which
  creates and returns a `Mezzio\Router\FastRoute` instance composing the
  two services.
