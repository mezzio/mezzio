...
>
> Mezzio 3.0 only supports [PSR-15 middleware and request handlers](https://www.php-fig.org/psr/psr-15/).
...
> function `Laminas\Stratigility\doublePassMiddleware()`) in order to decorate such
> middleware for use in Mezzio 3.
...
mezzio-tooling provides a tool for generating factories based on
reflecting a class; we'll use that to generate our factory:

```bash
$ composer mezzio factory:create "App\\Handler\\HelloHandler"
```
